...
**Flöde:**
1. Kod pushas till `main` eller `develop`.
2. GitHub Actions kör `npm ci` och `npm test`.
3. Vid lyckade tester loggar workflow in till Azure via OIDC.
4. Deploy sker till App Service med rätt behörighet.

---

### 4.2 Skapa federerad inloggning (OIDC)
1. Gå till **Azure Portal → App Service → Identity → System assigned** och slå på **"Status: On"**.  
2. Gå till **Azure Active Directory → App registrations**, skapa en ny app för GitHub Actions.  
3. Under **Federated credentials**, skapa en ny credential:
   - **Entity type:** `Branch`
   - **Branch name:** `main`
   - **Repository:** `CA-Grupp-5/Backend-Chas-Advance`
   - **Name:** `github-oidc-deploy`
4. Kopiera `clientId`, `tenantId` och `subscriptionId` (behövs i workflow).

---

### 4.3 GitHub Actions Workflow
Fullständig pipeline finns i repot:  
[`/.github/workflows/deploy.yml`](https://github.com/CA-Grupp-5/Backend-Chas-Advance/blob/Added-depart/devlived-features-for-packages/.github/workflows/deploy.yml)
...
| `DB_PASSWORD` | Databaslösenord (om inte satt direkt i Azure) |
